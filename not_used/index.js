...
const Index = () => {
	...
	const getData = async (city, direction, lastDoc) => {
		...
		if (!lastDoc) {
			homesRef = await firestore.collection('homes').orderBy('updated', 'desc').limit(perPage).get();
			setPage(1);
		...
		if (!homesRef.empty) {
			...
			let newHomes = [];
			homesRef.forEach(home => newHomes.push({ docId: home.id, ...home.data() }));
			setHomes(p => newHomes);
	...
	return (
		<div className="relative flex flex-col w-screen h-full flex-grow overflow-y-scroll antialiased">
			...
			<div className="mt-5 md:mt-20 pl-8 text-xl">
				<Link href={'/'}>
					<div className="ml-4 flex text-gray-600 items-center top-2 cursor-pointer">
						...
						<span className=" text-xl text-center">Homes in {city && stringParser(city)}</span>
					</div>
				</Link>
			</div>
			{loading && <LoadingPage />}
			...
			<div className="grid flex-grow flex-shrink-0 mb-16 pb-10 grid-cols-1 mt-8 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-12">
				{!loading &&
					homes &&
					homes.map(home => <HomeListCard property={home} city={city} key={home.title} />)}
			</div>

			{homes && (
				<div className="px-12 h-12 mb-20 mt-6 flex justify-center items-center flex-shrink-0 flex-grow">
					<div className="flex w-56 justify-between ">
						<PaginationButton hidden={page <= 1} onClick={getPrevious} direction="back" />
						<PaginationButton hidden={homes.length < 10} onClick={getNext} direction="next" />
					</div>
				</div>
			)}
