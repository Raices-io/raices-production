...
export async function createOrFindUser(uid, data) {
	...
	if (userRef.exists) {
		...
		user = { email, displayName, userType, profilePic, uid, languages, about, admin };
	} else {
		...
		try {
			await firestore
				.collection('users')
				.doc(uid)
				.set({ uid, ...data, userType: 'user', admin: false }, { merge: true });
			user = { ...data, userType: 'user', admin: false };
		} catch (e) {
			throw new Error(e);
		}
	}
