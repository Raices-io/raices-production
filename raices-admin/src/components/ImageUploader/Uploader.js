...
const UploadTask = ({ picture, pendingHomeId }) => {
	...
	useEffect(() => {
		...
		const photoRef = storageRef.child(`images/`).child(`homeImages/${pendingHomeId}/${storedName}`);
		try {
	...
	return (
		<Fragment>
			{url && url !== 'deleted' ? (
				...
				<div class="w-full h-auto block flex justify-center flex-grow bg-cover  bg-no-repeat bg-center">
					<FillerImage>
						{!url && progress !== 100 && <progress value={progress} max="100" />}
						{url == 'deleted' && 'deleted'}
					</FillerImage>
				</div>
			)}
...
const Uploader = ({ pendingHomeId }) => {
	...
	return (
		<div className=" mt-8 flex  self-center flex-col  antialiased">
			<input className="form-input" type="file" multiple={true} onChange={handleChange} />
			{files && files.map(picture => <UploadTask pendingHomeId={pendingHomeId} picture={picture} />)}
			<br />
		</div>
	);
};
